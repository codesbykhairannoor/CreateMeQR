...
// We will inject a resize listener for isMobile
const importsTarget = `import { Download, CheckCircle, BarChart2, Hash, Settings2, Layers, ScanLine, Smartphone, Zap, Code2 } from 'lucide-react';`;
const newImports = `import { Download, CheckCircle, BarChart2, Hash, Settings2, Layers, ScanLine, Smartphone, Zap, Code2 } from 'lucide-react';\nimport { useMediaQuery } from '../hooks/useMediaQuery'; // Wait, let's just use inline state since we don't know if useMediaQuery exists.`;

// Let's add the isMobile state inside the component
...
// Now we need to extract the Preview card JSX into a function so we can render it conditionally in two places.
// Let's put the function right before the `return (` statement.
...
// Now we replace the original Live Preview in Right Column with { !isMobile && <LivePreviewCard /> }
const rightColStart = `{/* Right Column: Live Preview (5 cols) */}\n          <div className="lg:col-span-5 flex flex-col gap-6">\n            <div className="bg-gradient-to-br from-blue-900 to-slate-900`;
const rightColEnd = `</button>\n              </div>\n            </div>\n          </div>`;

const startIndex = content.indexOf(`{/* Right Column: Live Preview (5 cols) */}`);
...
// And finally inject { isMobile && <LivePreviewCard /> } between Barcode Data and Visual Tuning
