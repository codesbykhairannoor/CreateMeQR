      ...
      <section className="relative pt-32 pb-16 lg:pt-40 lg:pb-24 border-b border-slate-200 dark:border-blue-900/30 transition-colors duration-300">
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10 text-center">
          ...
          <p className="text-xl md:text-2xl text-zinc-500 dark:text-zinc-400 max-w-2xl mx-auto mb-10 font-medium leading-relaxed">
            {t("scanqr.hero.desc", "Scan from your webcam or upload an image. 100% private, zero data is sent to our servers.")}
          </p>
        </div>
      </section>
            ...
            <div className="space-y-8">
              {!isScanning && !scanResult && (
                <div className="border-2 border-dashed border-blue-300 dark:border-blue-500/30 rounded-3xl p-12 text-center bg-blue-50 dark:bg-blue-500/5 hover:bg-blue-100 dark:hover:bg-blue-500/10 transition-colors">
                  ...
                  <h3 className="text-2xl font-bold text-zinc-900 dark:text-white mb-2">{t("scanqr.workspace.initCam", "Initialize Camera")}</h3>
              ...
              {scanResult && !isScanning && (
                <div className="border-2 border-dashed border-slate-200 dark:border-blue-500/30 rounded-3xl p-8 text-center bg-white dark:bg-blue-500/5 transition-colors">
                  ...
                  <h3 className="text-2xl font-bold text-zinc-900 dark:text-white mb-6">{t("scanqr.workspace.successTitle", "Scan Successful")}</h3>
                  <button 
                  ...
                  >
                    <Camera className="w-5 h-5 mr-2" /> {t("scanqr.workspace.startBtn", "Start Scanning")}
                  </button>
                </div>
              )}
              ...
              {!isScanning && (
                <div className="pt-8 border-t border-slate-200 dark:border-[#1e2d4a]">
                  <p className="text-center text-sm font-bold text-slate-500 dark:text-slate-400 mb-6 uppercase tracking-widest">{t("scanqr.workspace.orUpload", "Or upload an image")}</p>
                  <input 
                    type="file" 
                    accept="image/*" 
                ...
                {scanResult ? (
                  <div className="flex-1 flex flex-col">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      ...
                      >
                        {copied ? <span className="text-blue-600 dark:text-blue-400">{t("scanqr.workspace.copied", "Copied!")}</span> : <><Copy className="w-5 h-5" /> Copy Data</>}
                      </button>
                      ...
                      {isUrl(scanResult) ? (
                        ...
                        >
                          <ExternalLink className="w-5 h-5" /> {t("scanqr.workspace.openLink", "Open Link")}
                        </a>
                      ) : (
                        <div className="py-4 bg-slate-100 dark:bg-slate-800/50 text-slate-400 font-bold rounded-xl flex items-center justify-center gap-2 cursor-not-allowed">
                          {t("scanqr.workspace.notLink", "Not a link")}
                        </div>
                      )}
                    </div>
                  </div>
                  ...
                  <div className="flex-1 flex flex-col items-center justify-center text-center opacity-40">
                    ...
                    <p className="text-xl font-bold mb-2 text-slate-900 dark:text-white">{t("scanqr.workspace.waiting", "Waiting for scan...")}</p>
                    <p className="text-sm text-slate-600 dark:text-slate-400">{t("scanqr.workspace.waitingDesc", "Point your camera at a QR code or upload an image.")}</p>
                  </div>
                )}
              </div>
            </div>
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10 flex flex-col md:flex-row items-center gap-16">
          <div className="flex-1 space-y-8">
            ...
            <p className="text-xl text-blue-100 font-medium leading-relaxed">
              {t("scanqr.features.moreDesc", "Our advanced scanner instantly recognizes various payload types. It doesn't just read the text, it understands the context.")}
            </p>
            <ul className="space-y-4 text-white font-bold text-lg">
              <li className="flex items-center gap-3">
                ...
                {t("scanqr.features.moreList1", "WiFi Credentials Parsing")}
              </li>
              <li className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-full bg-white/20 flex items-center justify-center"><ShieldCheck className="w-4 h-4" /></div>
                {t("scanqr.features.moreList2", "vCard Contact Import")}
              </li>
              <li className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-full bg-white/20 flex items-center justify-center"><ShieldCheck className="w-4 h-4" /></div>
                {t("scanqr.features.moreList3", "Crypto Addresses")}
              </li>
            </ul>
          </div>
