...
const NEW_KEYS = {
  ...
  "catMessage": "Messaging",
  "catContact": "Contact",
  "catWeb": "Links & Web",
  "catFinance": "Finance & Store",
  "catPlaces": "Places & Events",
  "catMedia": "Media & Entertainment",
  "catData": "Files & Data"
};
...
async function run() {
  ...
  for (const lang of LANGS) {
    ...
    for (const k in NEW_KEYS) {
      if (!data.nav[k] || data.nav[k] === NEW_KEYS[k] && lang !== 'en') {
  ...
  console.log("Done translating new mega menu categories.");
}
run();
